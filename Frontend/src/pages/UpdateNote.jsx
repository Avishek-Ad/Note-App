...
const UpdateNote = () => {
  ...
  const {titles, descriptions, tagss, id} = location.state
  ...
  const updateData = () => {
    axios.patch(`https://note-app-khaki-five.vercel.app/api/v1/notes/${id}`,
      {
        title:title,
  ...
  return (
    <div className="m-auto border border-gray-300 rounded-md w-96 h-auto py-10 text-center relative top-20 dark:text-white dark:bg-slate-700 dark:border-slate-900">
      ...
      <form className="flex flex-col gap-5 justify-start"onSubmit={(e) => handleFormSubmit(e)}>
        ...
        <button type="Submit" className="mx-auto px-5 mt-5 border-2 rounded-md inline w-fit hover:bg-slate-200 dark:hover:bg-slate-600 dark:border-slate-900">Update</button>
      </form>
    </div>
  )
}
